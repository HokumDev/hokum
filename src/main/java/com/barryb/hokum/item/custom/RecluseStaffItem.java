...
public class RecluseStaffItem extends StaffItem {

    public RecluseStaffItem(Tier tier, int attackMod, float attackSpeedMod, Properties props) {
    ...
    public InteractionResult interactLivingEntity(ItemStack itemStack, Player player, LivingEntity target, InteractionHand interactionHand) {
        if (!player.getCooldowns().isOnCooldown(itemStack.getItem())) {
            target.addEffect(new MobEffectInstance(MobEffects.WITHER, 120, 1), player);
    ...
    public void appendHoverText(ItemStack p_41421_, @Nullable Level p_41422_, List<Component> components, TooltipFlag p_41424_) {
        components.add(Component.literal("Inflicts Wither when right clicking a mob").withStyle(ChatFormatting.ITALIC).withStyle(ChatFormatting.GRAY));
    }
